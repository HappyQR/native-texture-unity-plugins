...
{
    public static class NativeTextureD3D12
    {
        ...
        public static bool IsSupported
        {
            get
            {
                ...
                return s_hasCachedGraphicsDeviceType && s_cachedGraphicsDeviceType == GraphicsDeviceType.Direct3D12;
            }
        }
        ...
        public static void Initialize()
        {
            ...
            EnsureD3D12BackendOnMainThread();
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            return await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    {
                        throw new InvalidOperationException("Native Decode returned a null D3D12 RGBA buffer.");
                    }
        ...
        public static Task<NativeTextureD3D12Texture> CreateAsync(
        ...
        public static async Task<NativeTextureD3D12Texture> CreateAsync(
            byte[] rgba,
            int width,
            int height,
            bool linear = false,
            CancellationToken cancellationToken = default)
        {
            ...
            IntPtr nativeTexture = await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    {
                        throw new InvalidOperationException("Native Create returned a null D3D12 texture.");
                    }
        ...
        public static async Task<NativeTextureD3D12Texture> LoadTextureAsync(
            byte[] encodedData,
            bool linear = false,
            CancellationToken cancellationToken = default)
        {
        ...
        public static Task<NativeTextureD3D12Texture> LoadTextureAsync(
            byte[] encodedData,
            bool linear,
            bool flipY)
        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            NativeCreateResult nativeResult = await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    if (texture == IntPtr.Zero)
                    {
                        throw new InvalidOperationException("Native CreateFromEncoded returned a null D3D12 texture.");
                    }
        ...
        public static async Task<NativeTextureD3D12Texture> LoadTextureFromFileAsync(
            string filePath,
            bool linear = false,
            CancellationToken cancellationToken = default)
        {
        ...
        public static Task<NativeTextureD3D12Texture> LoadTextureFromFileAsync(
            string filePath,
            bool linear,
            bool flipY)
        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            NativeCreateResult nativeResult = await Task.Run(() =>
            {
                ...
                if (texture == IntPtr.Zero)
                {
                    throw new InvalidOperationException("Native CreateFromFile returned a null D3D12 texture.");
                }
        ...
        {
            NativeTextureReleaseScheduler.ReleaseExternalTexture("D3D12", texture, nativeTexture, Release);
        }

        private static async Task<NativeTextureD3D12Texture> WrapNativeTextureAsync(
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                return new NativeTextureD3D12Texture(texture, nativeTexture);
            }
            catch
            {
        ...
        private static void EnsurePluginLoadedOnMainThread()
        {
            ...
            if (!IsMainThread())
            {
                throw new InvalidOperationException("NativeTextureD3D12 plugin warmup must run on the Unity main thread.");
            }
        ...
        private static void EnsureD3D12BackendOnMainThread()
        {
            TryCaptureMainThreadContext();
            if (!IsMainThread())
            {
                throw new InvalidOperationException("NativeTextureD3D12 backend validation must run on the Unity main thread.");
            }

            RefreshGraphicsDeviceTypeOnMainThread();
            if (s_cachedGraphicsDeviceType != GraphicsDeviceType.Direct3D12)
            {
                throw new NotSupportedException(
                    $"NativeTextureD3D12 only supports Direct3D12. Current backend is {s_cachedGraphicsDeviceType}.");
            }
        }
        ...
        private static void EnsureMainThreadContext()
        {
            ...
            if (SynchronizationContext.Current == null)
            {
                throw new InvalidOperationException(
                    "NativeTextureD3D12 must be initialized from the Unity main thread before CreateAsync or disposal.");
            }
