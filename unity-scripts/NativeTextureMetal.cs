...
{
    public static class NativeTextureMetal
    {
        // Cancellation is cooperative around the native ABI because Decode/Create are synchronous native calls.
        ...
        public static bool IsSupported
        {
            get
            {
                ...
                return s_hasCachedGraphicsDeviceType && s_cachedGraphicsDeviceType == GraphicsDeviceType.Metal;
            }
        }
        ...
        public static void Initialize()
        {
            ...
            EnsureMetalBackendOnMainThread();
        }
        ...
        public static Task<NativeTextureMetalTexture> CreateAsync(
        ...
        public static async Task<NativeTextureMetalTexture> CreateAsync(
            byte[] rgba,
            int width,
            int height,
            bool linear = false,
            CancellationToken cancellationToken = default)
        {
            ...
            IntPtr nativeTexture = await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    {
                        throw new InvalidOperationException("Native Create returned a null Metal texture.");
                    }
        ...
        public static async Task<NativeTextureMetalTexture> LoadTextureAsync(
            byte[] encodedData,
            bool linear = false,
            CancellationToken cancellationToken = default)
        {
        ...
        public static Task<NativeTextureMetalTexture> LoadTextureAsync(
            byte[] encodedData,
            bool linear,
            bool flipY)
        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            NativeCreateResult nativeResult = await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    if (texture == IntPtr.Zero)
                    {
                        throw new InvalidOperationException("Native CreateFromEncoded returned a null Metal texture.");
                    }
        ...
        public static async Task<NativeTextureMetalTexture> LoadTextureFromFileAsync(
            string filePath,
            bool linear = false,
            CancellationToken cancellationToken = default)
        {
        ...
        public static Task<NativeTextureMetalTexture> LoadTextureFromFileAsync(
            string filePath,
            bool linear,
            bool flipY)
        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            NativeCreateResult nativeResult = await Task.Run(() =>
            {
                ...
                if (texture == IntPtr.Zero)
                {
                    throw new InvalidOperationException("Native CreateFromFile returned a null Metal texture.");
                }
        ...
        {
            NativeTextureReleaseScheduler.ReleaseExternalTexture("Metal", texture, nativeTexture, Release);
        }

        private static async Task<NativeTextureMetalTexture> WrapNativeTextureAsync(
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                return new NativeTextureMetalTexture(texture, nativeTexture);
            }
            catch
            {
        ...
        private static void EnsurePluginLoadedOnMainThread()
        {
            ...
            if (!IsMainThread())
            {
                throw new InvalidOperationException("NativeTextureMetal plugin warmup must run on the Unity main thread.");
            }
        ...
        private static void EnsureMetalBackendOnMainThread()
        {
            TryCaptureMainThreadContext();
            if (!IsMainThread())
            {
                throw new InvalidOperationException("NativeTextureMetal backend validation must run on the Unity main thread.");
            }

            RefreshGraphicsDeviceTypeOnMainThread();
            if (s_cachedGraphicsDeviceType != GraphicsDeviceType.Metal)
            {
                throw new NotSupportedException(
                    $"NativeTextureMetal only supports Metal. Current backend is {s_cachedGraphicsDeviceType}.");
            }
        }
        ...
        private static void EnsureMainThreadContext()
        {
            ...
            if (SynchronizationContext.Current == null)
            {
                throw new InvalidOperationException(
                    "NativeTextureMetal must be initialized from the Unity main thread before CreateAsync or disposal.");
            }
